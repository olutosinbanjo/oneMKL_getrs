...
void GETRS(cl::sycl::queue &queue_gpu, cl::sycl::queue &queue_host, cl::sycl::device &device_gpu, cl::sycl::device &device_host, cl::sycl::context &context_gpu, cl::sycl::context &context_host, double *A, double *b, std::int64_t size, std::int64_t A_size, std::int64_t b_size);
...
int main(int argc, char *argv[])
{
	...
	double *coeff_matrix = cl::sycl::malloc_shared<double>(coeff_matrix_size, queue_gpu);
	...
	GETRS(queue_gpu, queue_host, device_gpu, device_host, context_gpu, context_host, coeff_matrix, RHS, size, coeff_matrix_size, RHS_size);
...
{	
		...
		clock_t start, end;
			...
			std::int64_t *ipiv = cl::sycl::malloc_shared<std::int64_t>(ipiv_size, queue_gpu);
		       ...
		       try{
				...
				double *getrs_scratchpad = cl::sycl::malloc_shared<double>(getrs_scratchpad_size, device_host, context_host);

				// Perform factorization
				start = clock();

				auto getrf_done_event = oneapi::mkl::lapack::getrf(queue_gpu, m, n, A, lda, ipiv, getrf_scratchpad, getrf_scratchpad_size);
				...
				std::cout << "\n" << "Time taken by getrs USM heterogenous implementation: " << double(end - start)/double(CLOCKS_PER_SEC) << " seconds. " << "\n" << std::endl;
				// Wait until calculations are done
				getrs_done_event.wait_and_throw();

				//free scatch memory
				cl::sycl::free(getrf_scratchpad, context_gpu);
				cl::sycl::free(getrs_scratchpad, context_host);
