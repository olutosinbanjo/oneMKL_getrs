...
void GETRS(cl::sycl::queue &queue_gpu, cl::sycl::queue &queue_host, std::int64_t info, std::vector<double> A, std::vector<double> b, std::int64_t size, std::int64_t A_size, std::int64_t b_size);
...
//	cl::sycl::context context_gpu = queue_gpu.get_context();
	...
	start = clock();
	GETRS(queue_gpu, queue_host, INFO, coeff_matrix, RHS, size, coeff_matrix_size, RHS_size);
	end = clock();
