...
void GETRS(cl::sycl::queue &queue, std::vector<double> A, std::vector<double> b, std::int64_t size, std::int64_t A_size, std::int64_t b_size);
...
int main(int argc, char* argv[])
{
	...
	GETRS(queue, coeff_matrix, RHS, size, coeff_matrix_size, RHS_size);
		...
		clock_t start, end;
			...
			if(ipiv.empty()){
		       ...
		       try{
				...
				std::cout << "\n" << "Time taken by getrs buffer immplementation: " << double(end - start)/double(CLOCKS_PER_SEC) << " seconds. "<< std::endl;
				std::cout << "\n";
