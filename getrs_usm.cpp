...
void GETRS(cl::sycl::queue &queue, cl::sycl::device &device, cl::sycl::context &context, double *A, double *b, std::int64_t size, std::int64_t A_size, std::int64_t b_size);
...
int main(int argc, char *argv[])
{
	...
	GETRS(queue, device, context, coeff_matrix, RHS, size, coeff_matrix_size, RHS_size);
		...
		clock_t start, end;
			...
			std::int64_t *ipiv = cl::sycl::malloc_shared<std::int64_t>(ipiv_size, queue);
		       ...
		       try{
				...
				std::cout << "\n" << "Time taken by getrs USM implementation: " << double(end - start)/double(CLOCKS_PER_SEC) << " seconds. " << std::endl;
				std::cout << "\n";
